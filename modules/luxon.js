...
class DateTime {
   ...
   * @param {string|Zone} [opts.zone='local'] - use this zone if no offset is specified in the input string itself.
   ...
   * @param {Object} opts - options to affect the creation
   * @param {string|Zone} [opts.zone='local'] - convert the time to this zone.
   * Since the offset is always specified in the string itself, this has no effect on the interpretation of string,
   * merely the zone the resulting DateTime is expressed in.
   * @param {boolean} [opts.setZone=false] - override the zone with a fixed-offset zone specified in the string itself,
   * if it specifies one
   * @param {string} [opts.locale='system's locale'] - a locale to set on the resulting DateTime instance
   * @param {string} opts.outputCalendar - the output calendar to set on the resulting DateTime instance
   * @param {string} opts.numberingSystem - the numbering system to set on the resulting DateTime instance
   ...
   * @param {string} unit - The unit to go to the end of. Can be 'year', 'quarter',
   ...
   * for a more flexible formatting tool.
   * For a table of tokens and their interpretations, see [here](https://moment.github.io/luxon/#/formatting?id=table-of-tokens).
   * Defaults to en-US if no locale has been specified, regardless of the system's locale.
   * @param {string} fmt - the format string
   ...
   * Accepts the same options as the Intl.DateTimeFormat
   *  constructor and any presets defined by Luxon,
   * such as `DateTime.DATE_FULL` or `DateTime.TIME_SIMPLE`.
   * The exact behavior of this method is browser-specific,
   * but in general it will return an appropriate representation
   ...
   * @example DateTime.now().toLocaleString({ weekday: 'long', month: 'long', day: '2-digit' });
   * //=> 'Thursday, April 20'
   * @example DateTime.now().toLocaleString({ weekday: 'short', month: 'short', day:
   *  '2-digit', hour: '2-digit', minute: '2-digit' }); //=> 'Thu, Apr 20, 11:27 AM'
   * @example DateTime.now().toLocaleString({ hour: '2-digit', minute: '2-digit',
   *  hourCycle: 'h23' }); //=> '11:32'
   * @return {string}
   */
   ...
   * @example DateTime.utc().set({ hour: 7, minute: 34, seconds: 0,
   *  milliseconds: 0 }).toISOTime({ suppressSeconds: true }) //=> '07:34Z'
   * @example DateTime.utc().set({ hour: 7, minute: 34 }).toISOTime({ format: 'basic' }) //=>
   * '073419.361Z'
   * @example DateTime.utc().set({ hour: 7, minute: 34 }).toISOTime({ includePrefix: true }) //=>
   * 'T07:34:19.361Z'
   * @return {string}
   */
  toISOTime({
   ...
   *  The output is always expressed in GMT.
   ...
   * @param {Object} opts - options
   * @param {boolean} [opts.includeZone=false] - include the zone, such as 'America/New_York'.
   * Overrides includeOffset.
   * @param {boolean} [opts.includeOffset=true] - include the offset, such as 'Z' or '-04:00'
   * @param {boolean} [opts.includeOffsetSpace=true] -
   * include the space between the time and the offset,
   * such as '05:15:16.345 -04:00'
   ...
   * Overrides includeOffset.
   * @param {boolean} [opts.includeOffset=true] - include the offset, such as 'Z' or '-04:00'
   * @param {boolean} [opts.includeOffsetSpace=true] - include the space between the
   * time and the offset, such as '05:15:16.345 -04:00'
   * @example DateTime.utc(2014, 7, 13).toSQL() //=> '2014-07-13 00:00:00.000 Z'
   * @example DateTime.local(2014, 7, 13).toSQL() //=> '2014-07-13 00:00:00.000 -04:00'
   * @example DateTime.local(2014, 7, 13).toSQL({ includeOffset: false }) //=>
   ...
   * @example DateTime.now().toObject() //=> { year: 2017, month:
   * 4, day: 22, hour: 20, minute: 49, second: 42, millisecond: 268 }
   * @return {Object}
   */
  toObject(opts = {}) {
    if (!this.isValid) return {};
   ...
   * which compares the **local** calendar time.
   *  Use {@link DateTime#setZone} to convert one of the dates if needed.
   * @param {DateTime} otherDateTime - the other DateTime
   * @param {string} unit - the unit of time to check sameness on
   * @example DateTime.now().hasSame(otherDT, 'day'); //~> true
   * if otherDT is in the same current calendar day
   * @return {boolean}
   */
   ...
   *  Can only internationalize if your
   * platform supports Intl.RelativeTimeFormat. Rounds down by default.
   * @param {Object} options - options that affect the output
   * @param {DateTime} [options.base=DateTime.now()] - the DateTime to use as the basis
   * to which this time is compared.
   * Defaults to now.
   * @param {string} [options.style="long"] - the style of units, must be "long",
   * "short", or "narrow"
   * @param {string|string[]} options.unit - use a specific unit or array of units;
   * if omitted, or an array,
   * the method will pick the best unit. Use an array or one of "years",
   ...
   * Don't use in combination with {round: false} because the decimal
   * output will include the padding.
   ...
   * "yesterday" or "next month".
   * Only internationalizes on platforms that supports Intl.RelativeTimeFormat.
   * @param {Object} options - options that affect the output
   * @param {DateTime} [options.base=DateTime.now()] - the DateTime to use as the basis to which
   * this time is compared. Defaults to now.
   * @param {string} options.locale - override the locale of this DateTime
  ...
  /**
   * {@link DateTime#toLocaleString} format like '09:30:23 AM Eastern Daylight Time'.
  ...
  /**
   * {@link DateTime#toLocaleString} format like '10/14/1983, 9:30 AM'.
   * Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like '10/14/1983, 9:30:33 AM'.
   * Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'Oct 14, 1983, 9:30 AM'.
   *  Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'Oct 14, 1983, 9:30:33 AM'.
   * Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'Fri, 14 Oct 1983, 9:30 AM'.
   * Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'October 14, 1983, 9:30 AM EDT'.
   * Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'October 14, 1983, 9:30:33 AM EDT'.
   *  Only 12-hour if the locale is.
   * @type {Object}
   */
  ...
  /**
   * {@link DateTime#toLocaleString} format like 'Friday, October 14, 1983,
   * 9:30:33 AM Eastern Daylight Time'.
   *  Only 12-hour if the locale is.
   * @type {Object}
   */
